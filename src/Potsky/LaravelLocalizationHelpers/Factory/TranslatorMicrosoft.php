...
{
    protected Client $bingTranslator;
    ...
    public function __construct(array $config)
    {
        if ((isset($config['client_id'])) && (!is_null($config['client_id']))) {
            ...
            $env = $config['env_name_client_id'] ?? 'LLH_MICROSOFT_TRANSLATOR_CLIENT_ID';
        ...
        if ((isset($config['client_secret'])) && (!is_null($config['client_secret']))) {
            ...
            $env = $config['env_name_client_secret'] ?? 'LLH_MICROSOFT_TRANSLATOR_CLIENT_SECRET';
        ...
        $this->bingTranslator = new Client([
    ...
    public function translate($word, $toLang, $fromLang = null)
    {
        ...
        } catch (\MicrosoftTranslator\Exception $exception) {
            if (str_contains($exception->getMessage(), 'Unable to generate a new access token')) {
                throw $exception;
            }
        }
