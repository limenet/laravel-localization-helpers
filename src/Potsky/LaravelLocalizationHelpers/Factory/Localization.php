...
class Localization
{
     ...
     *               ...
     */
    public function convertLemmaToStructuredArray($lemmas, $dot_notation_split_regex, $level = -1): array
    {
        $lemmas_structured = [];
     ...
     *               ...
     */
    public function convertLemmaToFlatArray($lemmas): array
    {
        ...
        try {
            ...
            switch ($exception->getCode()) {
                ...
                case self::NO_LANG_FOLDER_FOUND_IN_YOUR_CUSTOM_PATH:
                    $this->messageBag->writeError('No lang folder found in your custom path: "'.$exception->getParameter().'"');
                    break;
            }
     ...
     * @return bool|mixed
     */
    private function evalString(int|string $str)
    {
        $a = false;
                ...
                foreach ($this->extractTranslationFromPhpFile($php_file_path, $trans_methods) as $k => $v) {
                    ...
                    if (is_string($a)) {
                        ...
                        if ($regex) {
                            ...
                            if (str_contains($real_value, $lemma)) {
                                $found = true;
                            }
                        }
